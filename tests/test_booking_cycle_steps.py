...
async def test_handle_slot_booking_actions_single_and_multiple(configured_jobs, test_engine):
    ...
    with Session(test_engine, expire_on_commit=False) as session:
        ...
        assert booking_prompts[0].message.text.startswith("Booked: Mon 23/03 18:00  Body Pump @ Center 1")
        assert "1. Tue 24/03 18:00  Body Pump @ Center 1" in choice_prompts[0].message.text
        assert "2. Tue 24/03 19:00  Body Pump @ Center 1" in choice_prompts[0].message.text
