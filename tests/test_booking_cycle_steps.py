...
def test_import_untracked_bookings_creates_pending_and_prompt(configured_jobs, test_engine):
    ...
    with Session(test_engine, expire_on_commit=False) as session:
        ...
        assert prompt.message.text.startswith(
            "Found a booking not tracked by the bot:\n- Mon 23/03 18:00  Body Pump @ Main Hall"
        )
...
async def test_handle_slot_booking_actions_single_and_multiple(configured_jobs, test_engine):
    ...
    with Session(test_engine, expire_on_commit=False) as session:
        ...
        assert booking_prompts[0].message.text.startswith("Booked: Mon 23/03 18:00  Body Pump @ Main Hall")
        assert "1. Tue 24/03 18:00  Body Pump @ Main Hall" in choice_prompts[0].message.text
        assert "2. Tue 24/03 19:00  Body Pump @ Main Hall" in choice_prompts[0].message.text
